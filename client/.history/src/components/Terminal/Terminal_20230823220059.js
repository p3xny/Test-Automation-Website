...
const Terminal = (props) => {
  ...
  const highlightWords = (message) => {
    ...
    const greenParts = message.split(
      new RegExp(`(${greenWords.join("|")})`, "g")
    );
    const redParts = message.split(new RegExp(`(${redWords.join("|")})`, "g"));
    const parts = [greenParts, redParts];

    for (let i = 0; i < parts.length; i++) {
      return parts[i].map((part, index) => {
        if (greenWords.includes(part)) {
          return (
            <span className={styles["highlightred-green"]} key={index}>
              {part}
            </span>
          );
        }
        if (redWords.includes(part)) {
          return (
            <span className={styles["highlightred-red"]} key={index}>
              {part}
            </span>
          );
        }
